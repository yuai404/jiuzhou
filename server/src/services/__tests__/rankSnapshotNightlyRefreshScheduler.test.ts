 ...
 * 2. 做什么：锁定调度时间锚点为每天本地时间凌晨 4 点，并要求初始化/停止都集中管理单一定时器。
 ...
 *
 * 数据流/状态流：
 * 读取源码 -> 断言调度器导入统一全量刷新入口 -> 断言下一次执行时间计算锚定凌晨 4 点
 * -> 断言初始化与停止都只管理单一定时器状态。
 *
 * 复用设计说明：
 ...
 * 2. 这里只约束“凌晨 4 点”和“统一刷新入口”，不锁日志文案，避免无关改动造成测试脆弱。
 */
...
test('凌晨调度器必须按本地时间凌晨 4 点计算下一次执行时间并管理单一定时器', () => {
  const source = readSource('../rankSnapshotNightlyRefreshScheduler.ts');

  assert.match(
    source,
    /nextRunAt\.setHours\(4,\s*0,\s*0,\s*0\);/u,
    '下一次执行时间必须锚定凌晨 4 点',
  );
  assert.match(
    source,
    /if\s*\(\s*nextRunAt\.getTime\(\)\s*<=\s*now\.getTime\(\)\s*\)\s*\{[\s\S]*?nextRunAt\.setDate\(nextRunAt\.getDate\(\)\s*\+\s*1\);[\s\S]*?\}/u,
    '若已超过当天凌晨 4 点，必须顺延到下一天',
  );
  assert.match(
    source,
