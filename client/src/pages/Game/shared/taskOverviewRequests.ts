 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：统一承接普通任务总览与摘要总览的并发请求合并，避免同一页面会话里的 Game 入口角标、任务弹窗等模块在同一时刻重复打到同一个接口。
 ...
 * - 输出：后端原始响应 `TaskOverviewResponse` / `TaskOverviewSummaryResponse`。
 *
 * 数据流/状态流：
 ...
 * 2. 详情接口与摘要接口是两个独立入口，必须拆开维护 inflight，不能混成单一状态，否则会让一侧错误阻塞另一侧刷新。
